...
typedef struct EbSvtAv1DecConfiguration {
    ...
    /* When set to 1, returns output pictures from all scalable layers present
     * in the bitstream.
     *
     * Default is 0, only one output layer is returned, defined by
     * operating_point parameter */
    uint32_t output_all_layers;

    /* Skip film grain synthesis if it is present in the bitstream. Can be used
    ...
    /**
     * @brief API signal for the library to know the channel ID (used for
     * pinning to cores)
     *
     * Min value is 0
    ...
    /**
     * @brief API signal for the library to know the active number of channels
     * being encoded simultaneously
     *
     * Min value is 1
     * Max value is 0xFFFFFFFF
     * Default is 1
    ...
    /* Decoder internal bit-depth is set to 16-bit even if the bitstream is
     * 8-bit
     *
     * Default is 0. */
 ...
 * @ *config_ptr     Pointer passed back to the client during callbacks, it will
 * be loaded with default parameters from the library. */
EB_API EbErrorType
 ...
 * @ *pComponentParameterStructure  Decoder and buffer configurations will be
 * copied to the library. */
EB_API EbErrorType svt_av1_dec_set_parameter(
    EbComponentType *svt_dec_component,
    EbSvtAv1DecConfiguration *
        pComponentParameterStructure);  // pComponentParameterStructure contents
                                        // will be copied to the library
...
EB_API EbErrorType svt_av1_dec_frame(EbComponentType *svt_dec_component,
                                     const uint8_t *data,
 ...
 *  been generated yet. Calling a decoding function is needed to generate more
 * pictures. */
