...
struct ZVideoFrame {
    ...
    ZVideoFrame(const uchar *data, int _width, int _height,
                uint64_t _timestamp = 0)
        : width(_width),
          height(_height),
          timestamp(_timestamp),
          yuv420pData(data, data + _width * _height * 3 / 2) {}

    ZVideoFrame(const std::vector<uchar> &dataVec, int _width, int _height,
                uint64_t _timestamp = 0)
        : yuv420pData(dataVec),
          width(_width),
          height(_height),
    ...
    ZVideoFrame(std::vector<uchar> &&dataVec, int _width, int _height,
                uint64_t _timestamp = 0)
        : yuv420pData(std::move(dataVec)),
          width(_width),
          height(_height),
          timestamp(_timestamp) {}

    ZVideoFrame(const std::shared_ptr<uchar> &dataPtr, int _width, int _height,
                uint64_t _timestamp = 0)
