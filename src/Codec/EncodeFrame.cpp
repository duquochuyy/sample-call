...
EncodeFrame::EncodeFrame(int width, int height)
    ...
    param.i_fps_num = 30;
    param.i_fps_den = 1;
    param.i_keyint_max = 30;  // khoảng cách xuất hiện khung hình I; khung hình
                              // mã hóa ko cần tham chiếu đến khung hình khác,
                              // giá trị này giảm -> tần suất khung hình I tăng
    param.b_intra_refresh = 1;
    param.rc.i_rc_method = X264_RC_CRF;
    param.rc.f_rf_constant =
        28;  // điều khiển tốc độ mã hóa, cân bằng giữa chất lượng và kích thước
...
void EncodeFrame::registerCallback(Callback *callback) { _callback = callback; }

void EncodeFrame::encodeYUV420ToH264(const ZVideoFrame &frame,
