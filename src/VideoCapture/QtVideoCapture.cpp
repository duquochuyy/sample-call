...
QtVideoCapture::QtVideoCapture(QObject *parent)
    : QObject(parent),
      camera(new QCamera(QCameraInfo::defaultCamera())),
      videoSurface(new QTVideoSurface(this)),
      frameCount(0) {
    QObject::connect(camera, &QCamera::stateChanged, this,
                     &QtVideoCapture::onCameraStateChanged);
    QObject::connect(camera, QOverload<QCamera::Error>::of(&QCamera::error),
                     this, &QtVideoCapture::onCameraError);
    QObject::connect(videoSurface, &QTVideoSurface::frameCapturedRawFormat,
                     this, &QtVideoCapture::onFrameCapturedRawFormat);
...
void QtVideoCapture::stop() { camera->stop(); }
