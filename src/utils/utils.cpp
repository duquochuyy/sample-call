...
void Utils::processNV12DatatToYUV420(const std::vector<uchar> &nv12Data, int width, int height, std::vector<uchar> &yuv420pData)
{
    ...
    for (int y = 0; y < uvHeight; ++y)
    {
        for (int x = 0; x < uvWidth; ++x)
        {
            ...
            uPlane[y * uvWidth + x] = uvPlane[uvIndex];
            vPlane[y * uvWidth + x] = uvPlane[uvIndex + 1];
        }
    }
}

void Utils::processNV12DatatToRGB(const std::vector<uchar> &nv12Data, int width, int height, QImage &image)
{
    int frameSize = width * height;
    const uchar *yPlane = nv12Data.data();
    const uchar *uvPlane = nv12Data.data() + frameSize;
