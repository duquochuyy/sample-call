...
void YuvWidget::initializeGL()
{
    ...
    if (!m_shaderProgram.addShaderFromSourceFile(QOpenGLShader::Vertex, ":/src/shaders/yuv_vertex.glsl"))
        qDebug() << "Failed to add vertex shader:" << m_shaderProgram.log();
    if (!m_shaderProgram.addShaderFromSourceFile(QOpenGLShader::Fragment, ":/src/shaders/yuv_fragment.glsl"))
...
//     int uvSize = ySize / 4;
//     qDebug() << "paint" << m_width << m_height << ySize << m_yuv420Data.size();

//     // Setup Y texture
//     glActiveTexture(GL_TEXTURE0);
//     glBindTexture(GL_TEXTURE_RECTANGLE, m_textureY);
//     glTexImage2D(GL_TEXTURE_RECTANGLE, 0, GL_LUMINANCE, m_width, m_height, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, m_yuv420Data.constData());
//     m_shaderProgram.setUniformValue("s_texture_y", 0);

//     // Setup U texture
//     glActiveTexture(GL_TEXTURE1);
//     glBindTexture(GL_TEXTURE_RECTANGLE, m_textureU);
//     glTexImage2D(GL_TEXTURE_RECTANGLE, 0, GL_LUMINANCE, m_width / 2, m_height / 2, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, m_yuv420Data.constData() + ySize);
//     m_shaderProgram.setUniformValue("s_texture_u", 1);

//     // Setup V texture
//     glActiveTexture(GL_TEXTURE2);
//     glBindTexture(GL_TEXTURE_RECTANGLE, m_textureV);
//     glTexImage2D(GL_TEXTURE_RECTANGLE, 0, GL_LUMINANCE, m_width / 2, m_height / 2, 0, GL_LUMINANCE, GL_UNSIGNED_BYTE, m_yuv420Data.constData() + ySize + uvSize);
...
void YuvWidget::paintGL()
{
    ...
    glActiveTexture(GL_TEXTURE0);
    glBindTexture(GL_TEXTURE_RECTANGLE, m_textureY);
    glTexImage2D(GL_TEXTURE_RECTANGLE, 0, GL_RED, m_width, m_height, 0, GL_RED, GL_UNSIGNED_BYTE, m_yuv420Data.constData());
    m_shaderProgram.setUniformValue("s_texture_y", 0);
    ...
    glActiveTexture(GL_TEXTURE1);
    glBindTexture(GL_TEXTURE_RECTANGLE, m_textureU);
    glTexImage2D(GL_TEXTURE_RECTANGLE, 0, GL_RED, m_width / 2, m_height / 2, 0, GL_RED, GL_UNSIGNED_BYTE, m_yuv420Data.constData() + ySize);
    m_shaderProgram.setUniformValue("s_texture_u", 1);
    ...
    glActiveTexture(GL_TEXTURE2);
    glBindTexture(GL_TEXTURE_RECTANGLE, m_textureV);
    glTexImage2D(GL_TEXTURE_RECTANGLE, 0, GL_RED, m_width / 2, m_height / 2, 0, GL_RED, GL_UNSIGNED_BYTE, m_yuv420Data.constData() + ySize + uvSize);
    m_shaderProgram.setUniformValue("s_texture_v", 2);
...
void YuvWidget::setFrameData(const QByteArray &yuv420pData, int width, int height)
{
